...
export default function AddPaymentScreen() {
  ...
  // Sync amount when customer data loads (context may arrive after first render)
  React.useEffect(() => {
  ...
  const validate = () => {
    ...
    if (customer && num > customer.balance)
      e.amount = `Amount exceeds balance of ${currency} ${customer.balance.toLocaleString()}`;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        {customer && (
          <Card
            style={[styles.customerBanner, { backgroundColor: colors.secondary }]}
          >
            ...
            <View>
              ...
              >
                Outstanding: {currency} {customer.balance.toLocaleString()}
              </Text>
            </View>
          </Card>
        )}
        ...
        >
          ...
          >
            Recording a PAYMENT (reducing debt)
          </Text>
        </View>
        ...
        {customer && customer.balance > 0 && (
          <TouchableOpacity
            onPress={() => setAmount(String(customer.balance))}
            style={[
              styles.fullPayBtn,
              { backgroundColor: colors.success + "15", borderRadius: colors.radius - 4, borderColor: colors.success + "30" },
            ]}
          >
            <Feather name="check-circle" size={16} color={colors.success} />
            <Text style={[styles.fullPayText, { color: colors.success, fontFamily: "Poppins_500Medium" }]}>
              Full payment: {currency} {customer.balance.toLocaleString()}
            </Text>
          </TouchableOpacity>
        )}
